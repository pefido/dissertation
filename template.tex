...
\title{Integration of browser-to-browser architectures with third party legacy cloud storage}
\subtitle{Science is beautiful}

% Autor
% use [f] for female and [m] for male
\authorname[m]{Pedro Filipe Nunes Durães}{Pedro Durães}
\authordegree{}
...
% use [f] for female and [m] for male
\adviser[m]{Nuno Manuel Ribeiro Preguiça}
{\\&Professor Associado}
...
% use [f] for female and [m] for male
\coadviser[m]{João Carlos Antunes Leitão}{\\&Professor Auxiliar}{\\& DI, FCT, Universidade NOVA de Lisboa}
...
\chapterfile{relatedWork}
\chapterfile{integrationDesign}
\chapterfile{integrationImplementation}
\chapterfile{evaluation}
\chapterfile{conclusion}
% \chapterfile{proposedWork}
...
\printbib[nottype=online]		% Print the bibliography
